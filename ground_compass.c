...
void pid_log_data(DistAngle* data, Queue* latring, Queue* lonring){
	updateAll(data, latring, lonring);
	Pid pid_data;
...
int main()
{
	...
	Go2Goal(&DistAngle_data, gps_latring, gps_lonring);

	double distance = dist_on_sphere(getLast(latring), getLast(lonring));
	...
	while (1)
	{
		...
		double dist2goal = dist_on_sphere(getLast(latring), getLast(lonring));
		...
		OriginLat = DestLat;
		OriginLon = DestLon;
		pid_log_data(&DistAngle_data, gps_latring, gps_lonring);
		compute_output(&pid_data);
		printf("pid_output = %d\n", pid_data.output);
